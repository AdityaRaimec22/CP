...
void writeMatrixToFile(FILE *file, complex **matrix, int rows, int columns) {
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < columns; j++) {
            fprintf(file, "%d,%d ", matrix[i][j].real, matrix[i][j].imaginary);
        }
        fprintf(file, "\n");
    }
}
...
int main() {
    ...
    fscanf(fileInput1, "%d %d", &rows1, &columns1);
    fscanf(fileInput2, "%d %d", &rows2, &columns2);
    ...
    readMatrixFromFile(fileInput1, matrix1, rows1, columns1);
    readMatrixFromFile(fileInput2, matrix2, rows2, columns2);

    // Perform matrix multiplication
    multiplyMatrices(matrix1, rows1, columns1, matrix2, rows2, columns2, result);

    // Write the result to the output file
    writeMatrixToFile(fileOutput, result, rows1, columns2);
